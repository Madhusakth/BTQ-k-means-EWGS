...
class STE_discretizer(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x_in, num_levels,indices=-1):
        if indices == -1:
            x = x_in * (num_levels - 1)
            x = torch.round(x)
            x = x / (num_levels - 1)
            ...
            #x = x_in * (num_levels - 1)
            x_in[indices] = x_in[indices] * (num_levels - 1)
            x = x_in

            x[indices] = torch.round(x[indices])
            x[indices] = x[indices] / (num_levels - 1)
            x_out=x
            #x_out[indices] = x[indices] / (num_levels - 1)
        return x_out
    @staticmethod
    def backward(ctx, g):
        return g, None, None #####remove last None
...
class QConv(nn.Conv2d):
    ...
    def weight_quantization(self, weight,indices=-1):
        ...
        if not self.baseline:
            ...
            weight = self.STE_discretizer(weight, self.weight_levels,indices)
    ...
    def act_quantization(self, x,indices=-1):
        ...
        if not self.baseline:
            ...
            x = self.STE_discretizer(x, self.act_levels,indices)
    ...
    def forward(self, x):
        ...
        Qweight = self.weight
        size = Qweight.shape[0]*Qweight.shape[1]*Qweight.shape[2]*Qweight.shape[3]
        import random
        indices = random.sample(range(0, size), int(size/10))
        #indices = -1
        idx_0 = Qweight.shape[0]
        idx_1 = Qweight.shape[1]
        idx_2 = Qweight.shape[2]
        idx_3 = Qweight.shape[3]

        Qweight = Qweight.reshape(idx_0*idx_1*idx_2*idx_3)

        if self.quan_weight:
            Qweight = self.weight_quantization(Qweight,indices)
