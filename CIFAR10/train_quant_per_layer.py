...
parser.add_argument('--per_layer', type=str2bool, default=False, help='per layer k-means')
...
def k_means_pq(weight, grad,name=None,bits=5):
    with torch.no_grad():
        print("Quantizing weight:", name, weight.shape)
        mean = False
        weighted = args.weighted #False
        ...
        if softmax:
            ...
            grad_np = grad_np.reshape(-1)/np.linalg.norm(grad_np) #norm_grad_np
            #grad_np = grad_np*1000  #multiply by a constant
...
for ep in range(args.epochs):
    ...
    if args.btq and ep >= 0:
        ...
        if args.per_layer:
            grad_total = {}
            for name, param in model.named_parameters():
                if ('layer' in name or 'weight' in name) and len(param.shape) >2 and (param.shape[0] == 64 and param.shape[1] ==64):
                    grad_total.update({name: abs(param.grad).sum().cpu().numpy()})
            print(grad_total)
            grad_total = {k: v for k, v in sorted(grad_total.items(), key=lambda item: item[1])}
            parition_bits = [2,2,5,8,8]
            grad_total_keys = list(grad_total.keys())
        ...
        for name, param in model.named_parameters():
            ...
            if ('layer' in name or 'weight' in name) and len(param.shape) >2:
                ...
                if args.sensitivity:
                    ...
                    bits = args.bits
                    if args.per_layer:
                        if name in grad_total:
                            bits = parition_bits[grad_total_keys.index(name)]
                        print(grad_total)
                        print("********************************", name, bits)

                    state_dict[name], layer_bits = k_means_pq(state_dict[name],param.grad,name,bits)
